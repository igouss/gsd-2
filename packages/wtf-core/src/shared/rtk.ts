...
const WTF_RTK_PATH_ENV = "WTF_RTK_PATH";
const WTF_RTK_DISABLED_ENV = "WTF_RTK_DISABLED";
const WTF_RTK_REWRITE_TIMEOUT_MS_ENV = "WTF_RTK_REWRITE_TIMEOUT_MS";
...
function getRewriteTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const configured = Number.parseInt(env[WTF_RTK_REWRITE_TIMEOUT_MS_ENV] ?? "", 10);
...
export function isRtkEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return !isTruthy(env[WTF_RTK_DISABLED_ENV]);
}
...
function getManagedRtkDir(env: NodeJS.ProcessEnv = process.env): string {
  return join(env.WTF_HOME || join(homedir(), PROJECT_DIR_NAME), "agent", "bin");
}
...
export function resolveRtkBinaryPath(options: ResolveRtkBinaryPathOptions = {}): string | null {
  ...
  const explicitPath = options.binaryPath ?? env[WTF_RTK_PATH_ENV];
