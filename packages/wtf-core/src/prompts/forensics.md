...
Then **offer GitHub issue creation**: "Would you like me to create a GitHub issue for this on igouss/wtf-3?"

**CRITICAL: The `github_issues` tool ONLY targets the current user's repository — it has no `repo` parameter. You MUST use `gh issue create --repo igouss/wtf-3` via the `bash` tool to file on the correct repo. Do NOT use the `github_issues` tool for this.**
...
ISSUE_URL=$(gh issue create --repo igouss/wtf-3 \
  --title "..." \
...
ISSUE_ID=$(gh api graphql -f query='{ repository(owner:"igouss",name:"wtf-3") { issue(number:'"$ISSUE_NUM"') { id } } }' --jq '.data.repository.issue.id')
TYPE_ID=$(gh api graphql -f query='{ repository(owner:"igouss",name:"wtf-3") { issueTypes(first:20) { nodes { id name } } } }' --jq '.data.repository.issueTypes.nodes[] | select(.name=="Bug") | .id')
