...
5. **Read the actual WTF source code** at `{{wtfSourceDir}}` to confirm or deny each hypothesis. Do not guess what code does — read it.
   ...
   **DB inspection:** If you need to check DB state as part of investigation, use `wtf_milestone_status` — never run `sqlite3 .wtf/wtf.db` or `node -e require('better-sqlite3')` directly. The engine holds a WAL write lock; direct access will either fail or return stale data.
...
Then **offer GitHub issue creation**: "Would you like me to create a GitHub issue for this on wtf-build/wtf-3?"

**CRITICAL: The `github_issues` tool ONLY targets the current user's repository — it has no `repo` parameter. You MUST use `gh issue create --repo wtf-build/wtf-3` via the `bash` tool to file on the correct repo. Do NOT use the `github_issues` tool for this.**
...
cat > /tmp/wtf-forensic-issue.md << 'WTF_ISSUE_BODY'
## Problem
...
---
*Auto-generated by `/wtf forensics`*
WTF_ISSUE_BODY

ISSUE_URL=$(gh issue create --repo wtf-build/wtf-3 \
  --title "..." \
  --label "auto-generated" \
  --body-file /tmp/wtf-forensic-issue.md)
rm -f /tmp/wtf-forensic-issue.md

# Step 2: Set issue type via GraphQL (gh issue create has no --type flag)
ISSUE_NUM=$(echo "$ISSUE_URL" | grep -oE '[0-9]+$')
ISSUE_ID=$(gh api graphql -f query='{ repository(owner:"wtf-build",name:"wtf-3") { issue(number:'"$ISSUE_NUM"') { id } } }' --jq '.data.repository.issue.id')
TYPE_ID=$(gh api graphql -f query='{ repository(owner:"wtf-build",name:"wtf-3") { issueTypes(first:20) { nodes { id name } } } }' --jq '.data.repository.issueTypes.nodes[] | select(.name=="Bug") | .id')
