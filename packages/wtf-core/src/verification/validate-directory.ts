/**
 * WTF Directory Validation — Safeguards against running in dangerous directories.
 *
 * Prevents WTF from creating .wtf/ structures in system paths, home directories,
...
export interface DirectoryValidationResult {
  /** Whether the directory is safe for WTF operations */
  safe: boolean;
...
/** Paths where WTF must never create .wtf/ — no override possible. */
...
/**
 * Validate whether a directory is safe for WTF to operate in.
 *
 * Checks in order:
...
export function validateDirectory(dirPath: string): DirectoryValidationResult {
  ...
  if (blockedPaths.has(normalized)) {
    return {
      safe: false,
      severity: "blocked",
      reason: `Refusing to run in system directory: ${normalized}. WTF must be run inside a project directory.`,
    };
  }
  ...
  if (normalized === resolvedHome) {
    return {
      safe: false,
      severity: "blocked",
      reason: `Refusing to run in your home directory (${normalized}). WTF must be run inside a project directory, not $HOME.`,
    };
  }
  ...
  try {
    ...
    if (entries.length > 200) {
      return {
        safe: false,
        severity: "warning",
        reason: `This directory has ${entries.length} entries, which suggests it may not be a project directory. Are you sure you want to initialize WTF here?`,
      };
    }
  } catch {
...
/**
 * Assert that a directory is safe for WTF operations.
