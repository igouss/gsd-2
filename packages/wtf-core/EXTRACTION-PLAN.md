# Phase 2: Extract harness-free files into wtf-core

## Context

We're extracting WTF from pi-mono into a standalone, harness-agnostic package.
`packages/wtf-core/` already exists with `src/harness-adapter.ts` (the adapter
...
into `packages/wtf-core/src/`, preserving the same relative directory structure.
...
Work on branch `feat/wtf-core-extraction` (already exists, checked out from main).
...
`src/resources/extensions/wtf/` to `packages/wtf-core/src/`, preserving
subdirectory structure. Create subdirectories as needed.

```bash
SRC=src/resources/extensions/wtf
DST=packages/wtf-core/src
...
In `packages/wtf-core/src/auto-prompts.ts`, find:
```typescript
...
called. The key constraint: **no imports from `@wtf/pi-coding-agent` in wtf-core**.

### Step 3: Verify no harness imports leaked in

Run this from the repo root:
```bash
grep -r "@wtf/pi-coding-agent\|@wtf/pi-tui\|@wtf/pi-ai" packages/wtf-core/src/ --include="*.ts"
```

This MUST return zero results.

### Step 4: Verify wtf-core compiles
...
- Some harness-free files may import from harness-coupled files (e.g. a utility
  ...
  1. Extracting the specific type/function that's needed into wtf-core
  2. Or adding it to the adapter interface
  3. Or moving the dependency too if it turns out to be harness-free after all

### Step 5: Update index.ts exports

Add key exports to `packages/wtf-core/src/index.ts` as you discover which
...
- Do NOT update imports in the extension directory to point to wtf-core yet
- Do NOT add any `@wtf/pi-*` dependencies to wtf-core's package.json
- Do NOT worry about making the full auto-loop work from wtf-core yet — that's Phase 3

### Success criteria

1. All 186 files + prompts/ + templates/ copied to `packages/wtf-core/src/`
2. Zero grep hits for `@wtf/pi-coding-agent`, `@wtf/pi-tui`, or `@wtf/pi-ai` in `packages/wtf-core/src/`
3. `npx tsc --noEmit` in `packages/wtf-core/` passes (or has only cross-package import errors from coupled files, which is expected and documented)
