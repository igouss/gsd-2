// WTF Extension — Persistent Notification Store
// Captures all ctx.ui.notify() calls and workflow-logger warnings to
// .wtf/notifications.jsonl so they survive context resets and session restarts.
...
export function appendNotification(
  ...
  try {
    const dir = join(_basePath, PROJECT_DIR_NAME);
...
function _readEntriesFromDisk(basePath: string): NotificationEntry[] {
  const filePath = join(basePath, PROJECT_DIR_NAME, FILENAME);
...
function _withLock<T>(basePath: string, fn: () => T): T {
  const lockPath = join(basePath, PROJECT_DIR_NAME, LOCKFILE);
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      mkdirSync(join(basePath, PROJECT_DIR_NAME), { recursive: true });
