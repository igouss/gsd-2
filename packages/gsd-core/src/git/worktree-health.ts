/**
 * Worktree Health — lifecycle status helpers for WTF-managed worktrees.
 *
 * Used by doctor-checks.ts for health audits and by worktree-command.ts
 * for the enhanced `/worktree list` display.
 *
 * Only inspects worktrees under .wtf/worktrees/ — WTF owns what WTF creates.
 */
...
/**
 * Compute health status for all WTF-managed worktrees.
 *
