...
export const SLICE_BRANCH_RE: RegExp = /^gsd\/(?:([a-zA-Z0-9_-]+)\/)?(M\d+(?:-[a-z0-9]{6})?)\/(S\d+)$/;

/** Matches gsd/quick/ task branches */
export const QUICK_BRANCH_RE: RegExp = /^gsd\/quick\//;

/** Matches gsd/ workflow branches (non-milestone, e.g. gsd/workflow-name/...) */
export const WORKFLOW_BRANCH_RE: RegExp = /^gsd\/(?!M\d)[\w-]+\//;
