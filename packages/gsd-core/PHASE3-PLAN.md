...
Phase 2 extracted ~186 harness-free files into `packages/gsd-core/src/`. The auto-loop
files (`auto/loop.ts`, `auto/phases.ts`, `auto/loop-deps.ts`, `auto/session.ts`, `auto/run-unit.ts`)
are currently **stubs** in gsd-core because they were harness-coupled in the original.
...
Work on branch `feat/gsd-core-extraction`.
...
### Step 1: Define `CoreLoopDeps` in `packages/gsd-core/src/auto/loop-deps.ts`
...
### Step 2: Update `AutoSession` in `packages/gsd-core/src/auto/session.ts`
...
### Step 3: Update `IterationContext` in `packages/gsd-core/src/auto/types.ts`
...
7. **Replace `deps.resolver.mergeAndExit(mid, ctx.ui)` → `deps.resolver.mergeAndExit(mid, deps.events)`** (~3 call sites)
   - This means the `WorktreeResolver` interface needs to accept `OrchestratorEventSink` instead of `ctx.ui`. Check `packages/gsd-core/src/worktree-resolver.ts` — if it uses `ctx.ui.notify()`, change to accept `OrchestratorEventSink`.
...
18. **The `importExtensionModule` calls** in `generateMilestoneReport` and `runUnitPhase`. These are pi-mono-specific dynamic imports. Replace with regular imports since the modules are already in gsd-core:
    ```typescript
    // FROM:
...
### Step 6: Remove `auto/run-unit.ts` from gsd-core

Delete `packages/gsd-core/src/auto/run-unit.ts` — unit execution is now handled by
`HarnessAdapter.dispatchUnit()`. The file is no longer needed in gsd-core.
...
Check `packages/gsd-core/src/worktree-resolver.ts`. If `mergeAndExit` and
...
# 3. No references to ExtensionAPI, ExtensionContext, ExtensionCommandContext in gsd-core
grep -r "ExtensionAPI\|ExtensionContext\|ExtensionCommandContext" packages/gsd-core/src/ --include="*.ts"
```
...
| File | Action |
|------|--------|
| `gsd-core/src/auto/loop-deps.ts` | Replace stub with full `CoreLoopDeps` interface |
| `gsd-core/src/auto/session.ts` | Add missing session fields |
| `gsd-core/src/auto/types.ts` | Remove ctx/pi from `IterationContext` |
| `gsd-core/src/auto/loop.ts` | Replace stub with real loop, adapted for CoreLoopDeps |
| `gsd-core/src/auto/phases.ts` | Create new file — real phases adapted for CoreLoopDeps |
| `gsd-core/src/auto/run-unit.ts` | Delete (replaced by adapter.dispatchUnit) |
| `gsd-core/src/harness-adapter.ts` | No changes (already defined in Phase 1) |
| `gsd-core/src/worktree-resolver.ts` | Possibly update to accept OrchestratorEventSink |
