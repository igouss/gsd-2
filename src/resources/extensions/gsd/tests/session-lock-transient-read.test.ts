...
test('readExistingLockDataWithRetry reads file normally', async () => {
  ...
  try {
    ...
    assert.strictEqual(result!.pid, process.pid, 'correct PID read');
    assert.strictEqual(result!.sessionFile, 'test-session.json', 'correct sessionFile read');
  } finally {
    rmSync(base, { recursive: true, force: true });
  }
});

test('readExistingLockDataWithRetry returns null for missing file', async () => {
  const base = mkdtempSync(join(tmpdir(), 'gsd-transient-'));
  mkdirSync(join(base, '.gsd'), { recursive: true });
  ...
  try {
    ...
    assert.strictEqual(result, null, 'null for truly missing file after retries');
  } finally {
    rmSync(base, { recursive: true, force: true });
  }
});

test('readExistingLockDataWithRetry recovers after transient rename', async () => {
  const base = mkdtempSync(join(tmpdir(), 'gsd-transient-'));
  mkdirSync(join(base, '.gsd'), { recursive: true });
  ...
  try {
    ...
    if (result) {
      assert.strictEqual(result.pid, process.pid, 'correct PID after recovery');
      assert.strictEqual(result.sessionFile, 'recovery-session.json', 'correct sessionFile after recovery');
    }
  } finally {
    rmSync(base, { recursive: true, force: true });
  }
...
test('readExistingLockDataWithRetry recovers from transient permission error', async () => {
  const base = mkdtempSync(join(tmpdir(), 'gsd-transient-'));
  mkdirSync(join(base, '.gsd'), { recursive: true });
  ...
  try {
    ...
    if (result) {
      assert.strictEqual(result.pid, process.pid, 'correct PID after permission recovery');
    }
...
test('getSessionLockStatus tolerates transient lock file unavailability', async () => {
  const base = mkdtempSync(join(tmpdir(), 'gsd-transient-'));
  mkdirSync(join(base, '.gsd'), { recursive: true });
...
test('readExistingLockDataWithRetry default params work for readable file', async () => {
  const base = mkdtempSync(join(tmpdir(), 'gsd-transient-'));
  mkdirSync(join(base, '.gsd'), { recursive: true });
